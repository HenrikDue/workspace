...
Create view [dbo].[vw_Nedskriv_Primo_Netto_old_141112]
as
...
+( select replace(left(Vaerdi, 7), '-', '') from [edw].[MD_Styringstabel] where parameter = 'UltimoDato') FraTil_tid,
sum(beholdning) Beholdning, sum(Vaerdi_GP) AnskVaerdi, sum(NedskrNetto) as  NedForPrinc, 
sum(NedskrNetto) as NedskrNetto, sum(Vaerdi_SP) Vaerdi_SP
