...
CREATE VIEW [dbo].[vw_NedskrivNetto_old_150805_Med_Manuel_Vurd]
-- 150522-1: Design rettet
-- 150727-1: Tilføjet manuel vurdering til sum
-- 150805-1: Påsat FraTil_Tid på join til manuel vurdering
AS
...
From (
		...
		,lnb.Dim_Materiale
		,lnb.Materiale
		,isnull(lnb.LNType ,'AUB') LNType, isnull(lnb.FLNType, 'AEPD') as FLNType
		...
		,lnb.AnskVaerdi
		-- 150727-1: Tilføjet manuel vurdering til summer
		,isnull(lnb.NedForPrinc, 0) + isnull(kn36.NedForPrinc, 0) + isnull(ul12.NedForPrinc, 0) + isnull(b.NedForPrinc, 0) as NedForPrinc
		,isnull(lnb.NedskrNetto, 0) + isnull(kn36.NedskrNetto, 0) + isnull(ul12.NedskrNetto, 0) + isnull(b.NedskrNetto, 0) as NedskrNetto
		,lnb.Beholdning
		,lnb.Vaerdi_SP
		...
		-- 150727-1: Tilføjet join for manuel vurdering
		full outer join dbo.vw_ManuelVurdering_2 b on lnb.Dim_Fabrik = b.Dim_Fabrik and lnb.Dim_Materiale = b.Dim_Materiale
			and b.FraTil_Tid = cast(( select replace(left(Vaerdi, 7), '-', '') from [edw].[MD_Styringstabel] where parameter = 'PrimoDato') + '-' 
