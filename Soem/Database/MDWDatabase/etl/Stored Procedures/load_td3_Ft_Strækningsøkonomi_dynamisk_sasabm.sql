...
CREATE proc [etl].[load_td3_Ft_Strækningsøkonomi_dynamisk_sasabm]  @model varchar(50), @periode varchar(50), @sasabmmodelsdatabase sysname
as
begin 
set  nocount on
...
exec ('insert into ods.td_Mxxxx_costelement select * from '+@sasabmmodelsdatabase+'.dbo.'+@model+'_Costelement' )
--select * from  ods.td_Mxxxx_costelement
...
--exec ('insert into ods.td0_Mxxxx_AccountCenter select a.*,d.name as Drivername from '+@sasabmmodelsdatabase+'.dbo.'+@model+'_AccountCenter a left outer join '+@sasabmmodelsdatabase+'.dbo.'+@model+'_driver d on a.driverid = d.id' )
--ændring 20150828 i forhold til nyt sasabm
--følgende kolonner er fjernet: 
--[a].[InventoryCost],   ---InventoryCost
--[a].[ConsumedBOCQuantity],   ---ConsumedBOCQuantity

exec ('Insert into ods.td0_Mxxxx_AccountCenter (
	[Id],
	[Type],
	[SourceId],
	[DestinationId],
	...
	[Revenue],
	[TotalDriverQuantityBasic],
	[TotalDriverQuantityCalculated],
	[UserTotalDriverQuantity],
	[SoldQuantity],
	[UserInputQuantity],
	[UserOutputQuantity],
...
SELECT 
 ...
 [a].[Name],   ---Name
 [a].[Note],   ---Note
 [a].[CostPrimaryEntered],   ---CostPrimaryEntered
 [a].[CostPrimaryAssigned],   ---CostPrimaryAssigned
 [a].[CostPrimaryBOC],   ---CostPrimaryBOC
 [a].[CostReceivedBOC],   ---CostReceivedBOC
 [a].[CostReceivedAssigned],   ---CostReceivedAssigned
 [a].[CostGivenBOC],   ---CostGivenBOC
 [a].[CostGivenAssigned],   ---CostGivenAssigned
 [a].[UnitCostEntered],   ---UnitCostEntered
 [a].[IdleCost],   ---IdleCost
 
 [a].[Revenue],   ---Revenue
 [a].[TotalDriverQuantityBasic],   ---TotalDriverQuantityBasic
 [a].[TotalDriverQuantityCalculated],   ---TotalDriverQuantityCalculated
 [a].[UserTotalDriverQuantity],   ---UserTotalDriverQuantity
 [a].[SoldQuantity],   ---SoldQuantity
 [a].[UserInputQuantity],   ---UserInputQuantity
 [a].[UserOutputQuantity],   ---UserOutputQuantity

 [a].[Measure],   ---Measure
 [a].[UnitCostType],   ---UnitCostType
 [a].[AllocatedCostIn],   ---AllocatedCostIn
 [a].[AllocatedCost],   ---AllocatedCost
 [a].[AssignedIdleQuantity],   ---AssignedIdleQuantity
 [a].[UnassignedCost],   ---UnassignedCost
 [a].[CalcError],   ---CalcError
 [a].[PublishName],   ---PublishName
 [a].[IsBehavior],   ---IsBehavior
 [a].[Dim1001],   ---Dim1001
 [a].[Dim1002],   ---Dim1002
 [a].[Dim1003],   ---Dim1003
 [a].[Dim1004],   ---Dim1004
 ...
 [a].[Dim1011],   ---Dim1011
 [a].[Dim1012],   ---Dim1012
 [d].[name] as Drivername ---Drivername
From '+@sasabmmodelsdatabase+'.dbo.'+@model+'_AccountCenter a left outer join '+@sasabmmodelsdatabase+'.dbo.'+@model+'_driver d on a.driverid = d.id' )
