...
:: /* konfigurerer log */
md %cd%\Log
SET LOGFILE=%cd%\LOG\Log_Stog_Rejsedata_%DATE:~6,4%%DATE:~3,2%%DATE:~0,2%_%TIME:~0,2%%TIME:~3,2%%TIME:~6,2%.txt
...
ECHO. >> %LOGFILE%
SQLCMD -S %DB_SERVER% -d %DB_NAVN% -E -Q "exec etl.run_etl_Stog_Rejsedata ''" >> %LOGFILE%
ECHO ******************************************************************************
ECHO.
%LOGFILE%
COLOR A0
