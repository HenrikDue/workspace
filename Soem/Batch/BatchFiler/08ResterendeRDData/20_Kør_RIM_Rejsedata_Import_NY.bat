...
SET LOGFILE=.\LOG\Log_RIM_Rejsedata_%DATE:~6,4%%DATE:~3,2%%DATE:~0,2%_%TIME:~0,2%%TIME:~3,2%%TIME:~6,2%.txt
...
FOR /f %%a in ('SQLCMD -S %DB_SERVER% -d %DB_NAVN% -E -Q "declare @return_value int; EXEC @return_value = etl.run_etl_RIM_RejseData; print @return_value;" -h -1') DO SET returvaerdi=%%a
