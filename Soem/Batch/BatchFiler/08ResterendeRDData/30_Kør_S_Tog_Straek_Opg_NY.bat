...
SET SOURCE_DRIVE=P:
SET SOURCE_PATH=P:\70_BI\Projects\Files\Stog_Rejsedata\
SET SOURCE_FILE1=StraekningsopgorelseSTog

SET DEST_PATH=\\%DB_SERVER%\files\%DB_NAVN%\Stog_Rejsedata\
SET FILE_EXT=.xlsx

:: /* konfigurerer log */
IF NOT EXIST .\Log MD .\Log
SET LOGFILE=.\LOG\Log_Stog_Rejsedata_%DATE:~6,4%%DATE:~3,2%%DATE:~0,2%_%TIME:~0,2%%TIME:~3,2%%TIME:~6,2%.txt
...
SQLCMD -S %DB_SERVER% -d %DB_NAVN% -E -Q "declare @periode varchar(50); select @periode = substring(value,1,6) from ods.CTL_Dataload where kilde_system = 'Alle' and Variable = 'Model_Periode'; print '*  S-tog str‘kningsopg›relse LoadPeriode: '+@periode"
ECHO *
ECHO ******************************************************************************
ECHO.
...
FOR /f %%a in ('SQLCMD -S %DB_SERVER% -d %DB_NAVN% -E -Q "declare @return_value int; EXEC @return_value = etl.run_etl_Stog_Rejsedata; print @return_value;" -h -1') DO SET returvaerdi=%%a
